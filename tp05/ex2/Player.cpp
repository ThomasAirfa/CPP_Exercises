...
bool Player::play(Player& p1, Player& p2, std::vector<Card> stake)
{
    ...
    else if (p2_card < p1_card)
    {
        std::shuffle(stake.begin(), stake.end(), std::default_random_engine(rd()));
        for (Card& c : stake)
            p1.push_front(std::move(c)); // potentielle copie
